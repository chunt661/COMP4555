...
//ikita
		...
		_Float2("Float 2", Float) = 0.68
		[HDR]_Emis("Emis", Color) = (0,0,0,0)
		_TopTile("TopTile", Float) = 1
...
/* UNITY: Original start of shader */
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			float temp_output_15_0 = (0.0 + (( ( ase_worldPos.y - objToWorld4.y ) + _Height ) - 0.0) * (1.0 - 0.0) / (_Gradient - 0.0));
			float clampResult11 = clamp( temp_output_15_0 , 0.0 , 1.0 );
			float3 ase_worldNormal = WorldNormalVector( i, float3( 0, 0, 1 ) );
			float4 triplanar32 = TriplanarSamplingSF( _TopTexture0, ( ase_worldPos + ( float3(1,0,1) * fmod( _Time.y , 5.0 ) ) ), ase_worldNormal, 1.0, ( _TopTile * float2( 1,1 ) ), 1.0, 0 );
			float temp_output_26_0 = step( _Float2 , clampResult11 );
			float4 clampResult28 = clamp( ( ( 1.0 - temp_output_15_0 ) * triplanar32 * temp_output_26_0 ) , float4( 0,0,0,0 ) , float4( 1,1,1,1 ) );
			o.Emission = ( ( tex2D( _TextureSample0, uv_TextureSample0 ) * clampResult11 ) + ( clampResult28 * _Emis ) ).rgb;
